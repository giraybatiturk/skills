...
**Measure, don't guess.** Apply the `/real-module-audit` inventory approach: existing endpoints, permission actions, error codes, screens, state coverage. Whatever you can't measure (backend contract, third-party behaviour), write **"could not measure"** in the report and name who verifies it.
