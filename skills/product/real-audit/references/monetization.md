...
6. **Measurement gaps:** for each unresolved commercial question state the hypothesis, the instrumentation that is missing and the decision it blocks. Do not design the pricing change, the packaging or the experiment; that is the reader's call. Without baseline and sample-size inputs, do not invent uplift, sample counts, run duration or revenue forecasts. Format user follow-up work as validation tasks in `reporting.md`; small usability rounds can reveal price/offer comprehension problems but cannot establish willingness to pay, an optimal price or conversion uplift.
...
- **Price architecture:** describe the structure actually in use and where it fits the product's usage/value pattern badly, citing the observed effect on light or heavy users and on service costs. Report the mismatch; selecting a different structure is the reader's decision, not the audit's.
